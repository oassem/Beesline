...
# Second Phase: connect with Laravel API and customize missing functions in API 
> Add Product :
  1. connection is set :) but I need to customize Storage in laravel to upload images :).
  2. Open Backend Folder in ProductController, in store method :) add how handle image from frontend :) 
  3. before we need request to handle `store&UpdateProductRequest` so we make one `php artisan make:request store&UpdateProductRequest` 
